...
<head>
    ...
    <link rel="stylesheet" href="../css/menuFeedbacks.css">
...
<body>
    ...
    <main>
        ...
        <div class="feedback{{$loop->first ? ' first-feedback' : ''}}">
            <img src="{{asset($feedback->pictureLink)}}" alt="profil">
            <p class="name">{{$feedback->name}}</p>
            <p class="date">{{$feedback->date}}</p>
            <p class="comment">{{$feedback->review}}</p>
        </div>
        @endforeach
        @else
        <h3 class="no-comment">there is no comment for this menu</h3>
        @endif
    </main>
</body>
