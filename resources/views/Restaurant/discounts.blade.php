...
<body>
    ...
    <main>
        ...
        <div class="menu">
            <form action="{{route('Restaurant.changeDiscount',['menu_id'=>$menu->id])}}" method="post">
                @csrf
                <p>menu: {{$menu->plat}}</p>
                <label for="percentage">Discount (%)</label>
                <input type="number" name="percentage" max="100" min="0" id="percentage" value="{{$menu->discount}}">
                <p>price:{{$menu->price *((100-$menu->discount)/100)}} {{$menu->currency}}</p>
